...
const usePrivyCrossAppEIP1193 = ({ testnet = false }: { testnet: boolean }) => {
    ...
    const handleRequest = useCallback(async (request: any) => {
        ...
        switch (method) {
            ...
            case 'personal_sign': {
                console.log(params);
                return await signMessage(fromHex(params[0], 'string'), { address: params[1] });
            }
            default:
...
export { usePrivyCrossAppEIP1193 };
